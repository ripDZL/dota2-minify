...
- Remove Foilage must remain blacklist-only. Never ship `Minify/mods/Remove Foilage/manifest.json` or `Minify/mods/Remove Foilage/maps/dota.vpk`. Exact `tree_oak_leaves_05.vmat_c` and `tree_oak_leaves_05.vmdl_c` blacklist entries were restored from pre-`8afd759b` blob `1ea8db8b3f336609e67c2a924c8f6bf762e58cfd`.
...
- Remaining materialization includes UI/check boxes, beveled Obsidian + Ember theme, Settings, patch integration/glue, window/gui/__main__ integration, D2PFX browser UI, profile/import UI, fork regression tests, and fork beta version identification.
