"""Local-only tooling for the private Remove Foliage _09 human smoke."""
...
Test setup:
1. Keep the production "Remove Foliage" mod UNCHECKED.
2. Select only "Remove Foliage - Private Alias Smoke" for this foliage test.
3. Patch normally.
4. In Dota, verify:
...
def _prepare_output(source_mod: Path, output_dir: Path) -> None:
    ...
    if source_real == output_real or source_real in output_real.parents:
        raise ValueError("Smoke output must not replace or live inside the production Remove Foliage mod.")
...
def build_smoke_mod(source_mod: Path, stock_bytes: bytes, output_dir: Path) -> dict[str, object]:
    ...
    if not source_mod.is_dir():
        raise FileNotFoundError(f"Production Remove Foliage mod not found: {source_mod}")
    ...
    metadata: dict[str, object] = {
        "purpose": "Remove Foliage private _09 alias human smoke",
