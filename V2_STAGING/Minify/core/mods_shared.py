...
def get_conflicting_categories() -> set[str]:
        ...
        if plugins_dir and os.path.exists(plugins_dir):
            for entry in os.listdir(plugins_dir):
                ...
                if not os.path.isfile(manifest):
                    continue
                cfg = config.read_json_file(manifest)
                if not isinstance(cfg, dict):
                    continue
                extra = cfg.get("category_conflicts") or cfg.get("conflicting_categories")
                if isinstance(extra, (list, set, tuple)):
                    categories.update(str(value).casefold() for value in extra if value)
    except Exception:
        pass
    # Terrain conflicts are handled by the fork collision/yield layer.
...
def get_state(mod):
    ...
    return bool(states.get(mod, False)) if isinstance(states, dict) else False
...
def scan_mods():
    ...
    for mod in sorted(os.listdir(base.mods_dir), key=str.casefold):
        ...
        if is_ignored_folder(mod) or not os.path.isfile(mod_path) or not mod.lower().endswith(".vpk"):
            continue
