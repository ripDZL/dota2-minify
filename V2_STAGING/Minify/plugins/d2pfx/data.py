...
def validate_public_https_url(url: str) -> None:
    """Compatibility alias for the shared v2 security policy."""
    security.validate_public_https_url(url)


def safe_download_filename(url: str, allowed_extensions=None) -> str:
    return security.safe_download_filename(url, allowed_extensions)
...
class DataManager:
    ...
    def download_file(
        ...
        return fs.download_file(
            ...
            max_bytes=max_bytes,
            url_validator=security.validate_public_https_url,
            max_redirects=5,
        )
    ...
    def fetch_gz_json(self, filename, force_refresh=False):
        ...
        try:
            fd, compressed_path = tempfile.mkstemp(prefix=".d2pfx-catalogue-", suffix=".gz", dir=self.cache_dir)
            os.close(fd)
            ...
            info = os.stat(compressed_path, follow_symlinks=False)
            if not stat.S_ISREG(info.st_mode) or info.st_size > security.D2PFX_MAX_MANIFEST_BYTES:
                raise ValueError("Compressed D2PFX catalogue exceeds the safety limit.")
            with open(compressed_path, "rb") as file:
                compressed = file.read(security.D2PFX_MAX_MANIFEST_BYTES + 1)
