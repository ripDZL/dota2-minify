...
def resolve_dependencies(retries=0):
    """
    Download verified helper dependencies when an exact supported asset exists.

    Unknown architectures are PATH-only. Downloaded archives are checked against
    the pinned SHA-256 table before extraction.
    """
    try:
        ...
        if not os.path.exists(constants.rg_executable):
            if not constants.rg_latest:
                ...
                if fs.download_file(constants.rg_latest, archive_path, tag):
                    ...
                    if success:
                        fs.move_path(os.path.join(archive_name, rg_binary_name), rg_binary_name)
